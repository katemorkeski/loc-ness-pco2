...
ATMf <- ATMf |> mutate(rho_log = log10(rhodamine_ppb+0.5))
...
# # instead of this, need time
# atm_label <- ATMf$Type
# atm_label <- as.data.frame(atm_label)
# 
# write_csv(atm_label, here('loc-02-input/atm-sequence-list.csv'))
# 
# atmavg <- ATMf |> group_by(sequence) |> summarise(avg = mean(CO2ppm))
# 
# first_time <- stds |> group_by(sequence) |> summarise(first(date_time_utc)) |> select(-sequence)
# 
# stdavg <- cbind(stdavg, first_time)
